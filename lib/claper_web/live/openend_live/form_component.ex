defmodule ClaperWeb.OpenendLive.FormComponent do
  ...
  def update(%{form: form} = assigns, socket) do
    changeset = Openend.change_form(form)
  ...
  def handle_event("delete", %{"id" => id}, socket) do
    ...
    {:ok, _} = Openend.delete_form(socket.assigns.event_uuid, openend)
  ...
  def handle_event("validate", %{"form" => form_params}, socket) do
    changeset =
      socket.assigns.form
      |> Openend.change_form(form_params)
  ...
  def handle_event("add_field", _params, %{assigns: %{changeset: changeset}} = socket) do
    {:noreply, assign(socket, :changeset, changeset |> Openend.add_form_field())}
  end
  ...
  def handle_event(
    ...
    {:noreply, assign(socket, :changeset, changeset |> Openend.remove_form_field(form_field))}
  end

  defp save_form(socket, :edit, form_params) do
    case Openend.update_form(
  ...
  defp list_forms(assigns) do
    Openend.list_forms(assigns.presentation_file.id)
  end
end
