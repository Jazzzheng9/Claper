    ...
    <div>
      ...
      <div id="extended-form" class="bg-black w-full py-3 px-6 text-black shadow-lg rounded-md">
        ...
        <div>
          ...
          <%= if ! is_nil(@current_form_submit) do %>
          <button class="px-3 py-2 text-white font-semibold bg-gray-500 rounded-md my-5 cursor-default"><%= gettext "Submit" %></button>
          <% else %>
          <button phx-click="submit" phx-disable-with="..." class="px-3 py-2 text-white font-semibold bg-primary-500 hover:bg-primary-600 rounded-md my-5"><%= gettext "Submit" %></button>
          <% end %>
          </div>
      </div>
    </div>
