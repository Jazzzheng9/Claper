defmodule Lti13.Tool.ContextRoles do
  ...
  """
  @spec has_role?(Lti13_User.t(), any(), ContextRole.t()) :: boolean()
  def has_role?(user, context, role) when is_struct(user) do
    roles = Lti13_User.get_context_roles(user, context)
    Enum.any?(roles, fn r -> r.uri == role.uri end)
  end
  ...
  """
  @spec has_roles?(Lti13_User.t(), any(), [ContextRole.t()], :any) :: boolean()
  def has_roles?(user, context, roles, :any) when is_struct(user) and is_list(roles) do
    context_roles = Lti13_User.get_context_roles(user, context)
  ...
  @spec has_roles?(Lti13_User.t(), any(), [ContextRole.t()], :all) :: boolean()
