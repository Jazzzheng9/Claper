defmodule Lti13.Tool.PlatformRoles do
  ...
  """
  @spec has_role?(Lti13_User.t(), PlatformRole.t()) :: boolean()
  def has_role?(user, role) do
    roles = Lti13_User.get_platform_roles(user)
    Enum.any?(roles, fn r -> r.uri == role.uri end)
  end
  ...
  """
  @spec has_roles?(Lti13_User.t(), [PlatformRole.t()], :any) :: boolean()
  def has_roles?(user, roles, :any) when is_list(roles) do
    user_roles = Lti13_User.get_platform_roles(user)
  ...
  @spec has_roles?(Lti13_User.t(), [PlatformRole.t()], :all) :: boolean()
